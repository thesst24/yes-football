...
export class CheckIn {
  ...
  loadSeason() {
    this.http.get('http://localhost:3000/api/seasons/' + this.seasonId).subscribe({
      next: (res) => {
  ...
  loadNextSessionOfSeason() {
    this.http.get<any[]>('http://localhost:3000/api/sessions/season/' + this.seasonId).subscribe({
      next: (sessions) => {
